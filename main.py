...
def run_pipeline():
    ...
    if not model_loaded:
        ...
        # If model loaded, generate predictions on test set for review
        print("Generating predictions using loaded model...")
        metrics, test_df = forecaster.predict_test(processed_df, split_date=SPLIT_DATE)
        print(f"Model Metrics on Test Data: {metrics}")

    # 4. Generate Predictions on Test Set (Visualization)
    print("Generating predictions for validation/review...")
    
    if test_df is None or test_df.empty:
        print("No data in test set (split date might be too recent).")
        # Ensure X_test exists even if empty to avoid crashes if we proceeded
        X_test = pd.DataFrame(columns=forecaster.features)
    else:
